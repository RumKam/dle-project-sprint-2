...
2. **Предобученная трансформерная модель `distilgpt2`** – используется без дообучения для генерации продолжений.
...
## Бизнес-контекст
Проект имитирует реальную задачу из соцсетевого приложения. Необходимо добавить функцию автодополнения текстовых постов. Из-за ограничений мобильных устройств важно оценить, можно ли обойтись лёгкой моделью LSTM или же качество тяжёлой модели трансформера настолько выше, что стоит адаптировать её под мобильную платформу.
...
- Загрузка датасета [Sentiment140](https://www.kaggle.com/datasets/kazanova/sentiment140).
...
- Реализация цикла обучения с подсчётом функции потерь.
...
- Подбор параметров генерации (`top_p`, `temperature`).

### Этап 5. Формулирование выводов
- Сравнение метрик двух моделей.
- Анализ примеров генераций, а именно качество, связность, соответствие контексту.
...
Дополнительно оценивается качество визуально на нескольких примерах.
...
│
├── solution.ipynb                    # Ноутбук с итоговым решением (вызовы функций, обучение, визуализация, выводы)
...
4. Выполните по порядку ячейки в `solution.ipynb`.
