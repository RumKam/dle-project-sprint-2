...
4. Запустите предобработку данных (пример):
   ```bash
...
### LSTM модель
- **ROUGE-1 на валидации:** ...  
- **ROUGE-2 на валидации:** ...  
- **Пример генерации:**  
  *Вход:* "я сегодня очень"  
  *Продолжение:* "рад вас видеть"  
...
- **Пример генерации:**  
  *Вход:* "я сегодня очень"  
  *Продолжение:* "устал, но доволен"  
- **Время инференса на CPU:** ...  

### Сравнение
| Модель      | ROUGE-1 | ROUGE-2 | Примеры качества | Примечание |
|-------------|---------|---------|-------------------|------------|
| LSTM        | ...     | ...     | ...               | легковесная |
| Transformer | ...     | ...     | ...               | тяжелее     |
...
- **Какая модель показала лучшее качество?**  
  Ожидается, что трансформер даст более высокие метрики и более связные тексты, но потребует больше ресурсов.
- **Рекомендации:**  
  - Если ограничения памяти/скорости критичны, можно использовать LSTM (возможно, с дообучением).  
...
- **Финальное качество лучшей модели на тестовом датасете:**  
  - ROUGE-1: ...  
  - ROUGE-2: ...  
  - Примеры генераций: ...  
